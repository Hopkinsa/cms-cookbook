...
describe('AmendRecipeComponent', () => {
  it('initializes and updates image/tags/ingredients/steps', () => {
    ...
    const mockRecipeService: any = { updateRecipe: jest.fn(() => of({ completed: true })), createRecipe: jest.fn(() => of({ completed: true })) };

    const mockRoute: any = { snapshot: { data: { id: 1 } } };
    const mockRouter: any = { navigate: jest.fn() };

    TestBed.configureTestingModule({ imports: [AmendRecipeComponent], providers: [{ provide: SignalService, useValue: mockSignal }, { provide: RecipeService, useValue: mockRecipeService }, { provide: Router, useValue: mockRouter }, { provide: ActivatedRoute, useValue: mockRoute }] });
  ...
  it('save - update and create flows and back navigation', () => {
    ...
    const mockSignal: any = { recipe: () => recipe, recipe: { set: jest.fn() }, canEdit: jest.fn(), feedbackMessage: { set: jest.fn() }, returnTo: { set: jest.fn(), value: () => 'recipes' } };
    const mockRecipeService: any = { updateRecipe: jest.fn(() => of({ completed: true })), createRecipe: jest.fn(() => of({ completed: true })) };
    ...
    // update path
    TestBed.configureTestingModule({ imports: [AmendRecipeComponent], providers: [{ provide: SignalService, useValue: mockSignal }, { provide: RecipeService, useValue: mockRecipeService }, { provide: Router, useValue: mockRouter }, { provide: ActivatedRoute, useValue: mockRouteUpdate }] });
    ...
    TestBed.configureTestingModule({ imports: [AmendRecipeComponent], providers: [{ provide: SignalService, useValue: mockSignal }, { provide: RecipeService, useValue: mockRecipeService }, { provide: Router, useValue: mockRouter }, { provide: ActivatedRoute, useValue: mockRouteCreate }] });
