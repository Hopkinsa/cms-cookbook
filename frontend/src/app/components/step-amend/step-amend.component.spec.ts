...
describe('StepAmendComponent', () => {
  it('initializes from input and emits changes', () => {
    const fixture = TestBed.configureTestingModule({ imports: [StepAmendComponent] }).createComponent(StepAmendComponent);
    const comp = fixture.componentInstance;

    const mockSignal: any = () => ({ is_title: () => ({ value: () => false }), step: () => ({ value: () => 'initial step' }) });

    // set the underlying input signal function so component effects receive the value
    (comp as any).signalStep = () => ({ is_title: () => ({ value: () => false }), step: () => ({ value: () => 'initial step' }) });
